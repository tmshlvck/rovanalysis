...
def dispatch(param):
    (i, f, mapping, result_table, pinglist) = param

    pcap_restab1 = {k:False for k in pinglist}
    pcap_restab2 = {k:False for k in pinglist}
    pcap_mis1 = 0
    pcap_mis2 = 0
    for r in read_pcap(f):
        if r[1] == mapping[1][0]:
            if r[0] in pcap_restab1:
                pcap_restab1[r[0]] = True
            else:
                pcap_mis1 +=1
        elif r[1] == mapping[1][1]:
            if r[0] in pcap_restab2:
                pcap_restab2[r[0]] = True
            else:
                pcap_mis2 +=1
        else:
            pcap_mis1 +=1
    ...
    pcap_hit2 = 0
    for k in pinglist:
        if pcap_restab1[k] == True:
            pcap_hit1 += 1
        if pcap_restab2[k] == True:
            pcap_hit2 += 1
    print("x['%s']=(%d, %d, %d, %d)" %(f, pcap_hit1, pcap_mis1, pcap_hit2, pcap_mis2))


def analyze_mapping(mapping, result_table, pinglist, pool):
    (files_pairs, ipn_pairs) = mapping

    for i in (0,1):
        for f in files_pairs[i]:
            yield((i, f, mapping, result_table, pinglist))
...
def main():
    pinglist = list(readpinglist(pinglist_file))
    result_table1 = {k: [0,0] for k in pinglist}
    result_table2 = result_table1.copy()

    pool = Pool(processes=20)
    pool.map(dispatch, list(analyze_mapping(mapping1, result_table1, pinglist, pool)) + list(analyze_mapping(mapping2, result_table2, pinglist, pool)))
